...
```bash
rifteo-skills add high-severity-hunter
```
...
Install a severity-first, context-driven hunting mindset that reads the target before touching anything and builds the attack queue from what the target actually is.

- Derives attack priorities from program scope, tech stack, features, and enumeration — not a universal checklist
- Enforces the POC-or-kill rule throughout: no theoretical findings, no unconfirmed severity
- Hunts High and Critical first, treats them as one tier, then falls to Medium and Low only when the high-impact surface is covered
- Applies the kill list immediately — noise is dropped before any time is spent on it
- Evaluates chain potential on every finding before moving on
...
Tested on claude-sonnet-4-6 via Claude Code CLI. Same target, same prompt, same model — the only variable is whether the skill is loaded. Target: a simulated bug bounty program on a B2B fintech SaaS with payment processing, multi-tenant architecture, and a REST API.
...
Without the skill the agent produced a generic checklist — the same list it would produce for a social media app or a blog. Business logic, the highest-impact class for a payment SaaS, was never mentioned. XSS was prioritized before authorization. Missing headers and clickjacking were listed alongside critical payment flow risks. With the skill the agent read the target first: this is a payroll and wire transfer platform, money moves through it, and that context drives everything. The attack queue was derived from what the target does, not from a universal ordering.
...
<CardGroup cols={3}>
  ...
  <Card title="deadangle" href="/skills/deadangle">
    Re-examine every conclusion from multiple angles before delivery so findings hold up under scrutiny

    
  </Card>
  <Card title="cvss-scorer" href="/skills/cvss-scorer">
    Compute an exact CVSS v3.1 base score and vector from a confirmed finding
