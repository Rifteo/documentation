...
description: MCP server that gives AI agents live bug bounty program scope from HackerOne, Intigriti, YesWeHack, Bugcrowd, and Immunefi — plus access to all AuditGuard security contexts.
---
...
Stop copying scope tables by hand. The AuditGuard Context MCP server gives your AI agent live access to bug bounty program scope directly from the platform — in-scope assets, out-of-scope exclusions, bounty eligibility, and policy — before a single test is run.
...
- Loads AuditGuard security contexts (L1 overview or L2 full methodology) on demand
...
| Tool | What it does |
|---|---|
| `list_contexts` | List all available AuditGuard contexts with one-line summaries |
...
<CardGroup cols={3}>
  ...
  <Card title="API Security Review" href="/contexts/api-security-review">
    REST and GraphQL API security review methodology
  </Card>
